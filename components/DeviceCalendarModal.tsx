...
import { View, Text, StyleSheet, Modal, Pressable, TouchableOpacity, Linking, ActivityIndicator, ScrollView } from 'react-native';
...
import {
  requestCalendarPermission, getDeviceCalendars, listDeviceCalendarEvents,
  type DeviceCalendarInfo,
} from '../lib/deviceCalendar';

export default function DeviceCalendarModal({
  visible, onClose, granted: initialGranted, deniedForever,
}: {
  visible: boolean;
  onClose: () => void;
  // Read (not requested) by the caller right before opening — see household.tsx's
  // handleDeviceCalendarRow(). This modal only escalates to an actual request on user action.
  granted: boolean;
  ...
  // Re-sync to what the caller found each time the modal opens — a previous visit may have ended
  // with access denied, and the caller re-checks before every open.
  useEffect(() => {
    if (visible) setGranted(initialGranted);
  }, [visible, initialGranted]);
