...
// The theme-resolution algorithm hooks/useTheme.ts exposes to every in-app screen, pulled out
// as a plain function (no React, no Zustand) so it can also run in contexts that have neither —
// specifically the Android home-screen widget's headless task (widgets/HeimligWidget.tsx),
// which reads the same @heimlig/themeId / @heimlig/darkMode AsyncStorage keys directly rather
// than importing the store. useTheme() itself now just calls this with the store's values.
export function resolveThemeColors(themeId: string, darkMode: boolean): { colors: ColorPalette; isDark: boolean } {
